#!/bin/bash
# DSpark: DeepSeek DFlash backbone (FROZEN) + zero-init W2 + on-policy + HIGH CE.
# Changes from previous run: loss_fn={"ce":0.9,"tv":0.1}, epochs=20.
set -euo pipefail
...
LOG_DIR="/tmp/dspark_frozen_hce_ds_logs"; mkdir -p "$LOG_DIR"
...
DATASET="sharegpt"
OUTPUT_DIR="/tmp/dspark_frozen_hce_ds_output"
HIDDEN_STATES_PATH="/tmp/dspark_frozen_hce_ds_hs"
VLLM_PORT=8000; MAX_SAMPLES=5000; SEQ_LENGTH=4096; EPOCHS=10; LR=3e-4

BLOCK_SIZE=7; MAX_ANCHORS=512; TARGET_LAYER_IDS="2 10 18 26 34"; MASK_TOKEN_ID=151669
MARKOV_RANK=256; MARKOV_HEAD_TYPE="vanilla"
LOSS_FN='{"ce": 0.9, "tv": 0.1}'; CONFIDENCE_HEAD_ALPHA=1.0
ON_POLICY_WARMUP_RATIO=0.5
VLLM_NPUS="0"; TRAIN_NPUS="1,2,3"; NUM_TRAIN_NPUS=3

echo "=== DSpark frozen+on-policy+HCE (DeepSeek) started at $(date) ==="
echo "Key: frozen + zero_init_W2 + on_policy(0.5) + HCE(ce:0.9/tv:0.1) + 20ep"
