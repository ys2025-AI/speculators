#!/bin/bash
# DSpark: frozen DFlash backbone + sfa=false + high-CE (ce:0.9/tv:0.1), parametrized by init.
# Usage: bash dspark_frozen_hce.sh <label> <DFLASH_INIT> <DRAFT_CONFIG>
set -uo pipefail
LABEL="${1:?label required}"
DFLASH_INIT="${2:?dflash init required}"
DRAFT_CONFIG="${3:?draft-config required}"
