...
# block_size=7 (=> num_spec=7 for sfa=true), markov on, 10ep.
set -uo pipefail
...
LABEL="frozen_sfa_true_hce_ds_b7"
...
BLOCK_SIZE=7   # sfa=true -> num_spec = block_size = 7

echo "=== DSpark [${LABEL}] started at $(date) ==="
echo "DFLASH_INIT : $DFLASH_INIT (DeepSeek dflash) | block_size=$BLOCK_SIZE (sfa=true->num_spec=$BLOCK_SIZE) | freeze_backbone=YES | sfa=true | loss=$LOSS_FN | ep=$EPOCHS | markov=on"
...
echo "=== Step 3: Training (DeepSeek init, frozen, sfa=true, high CE, block7) ==="
