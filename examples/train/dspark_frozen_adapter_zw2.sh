#!/bin/bash
# DSpark: DeepSeek dflash init + FROZEN backbone + ZERO-INIT draft adapter (rank=256)
# + zero-init W2 (patched) + sfa=true + high-CE (ce:0.9/tv:0.1), 10ep.
# The draft adapter (zero-init, starts as identity -> 5.86) learns to realign the
# frozen DFlash backbone's logits for sfa=true (bridging the sfa mismatch).
set -uo pipefail
...
DFLASH_INIT="/home/model/dflash_qwen3_8b_block7_speculators"
DRAFT_CONFIG="/home/model/dflash_qwen3_8b_block7_speculators"
DATA_PATH="/tmp/dspark_from_dflash_output"
OUTPUT_DIR="/tmp/dspark_${LABEL}_output"
HIDDEN_STATES_PATH="/tmp/dspark_${LABEL}_hs"
VLLM_PORT=8000; EPOCHS=10; LR=3e-4; SEQ_LENGTH=4096
LOSS_FN='{"ce": 0.9, "tv": 0.1}'; BLOCK_SIZE=7

echo "=== DSpark [${LABEL}] started at $(date) ==="
echo "DeepSeek dflash init | frozen backbone | draft_adapter_rank=256 (zero-init) | zero-init W2 | sfa=true | loss=$LOSS_FN | ep=$EPOCHS"

if [ -f "$DATA_PATH/data-00000-of-00001.arrow" ]; then echo "=== Step 1: data found ==="; else
...
echo "=== Step 3: Training (frozen + draft_adapter=256 + zero-init W2 + sfa=true) ==="
ASCEND_RT_VISIBLE_DEVICES="1,2,3" "$TORCHRUN" --standalone --nproc_per_node 3 \
    ...
    --freeze-backbone --draft-adapter-rank 256 \
