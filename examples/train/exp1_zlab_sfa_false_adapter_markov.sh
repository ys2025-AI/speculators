#!/bin/bash
# Exp 1: z-lab DFlash + frozen + sfa=false + adapter(256,zero-init) + markov(256,zero-init W2)
set -uo pipefail
...
LABEL="exp1_zlab_sfa_false_adapter_markov"
...
DFLASH_INIT="/home/model/dspark_ckpt/dflash_zlab_speculators"
...
echo "=== Exp1 [${LABEL}] started at $(date) ==="
echo "z-lab DFlash | frozen | sfa=false | adapter=256(zero-init) | markov=256(zero-init W2) | loss=$LOSS_FN | ep=$EPOCHS"
if [ -f "$DATA_PATH/data-00000-of-00001.arrow" ]; then echo "data found"; else
