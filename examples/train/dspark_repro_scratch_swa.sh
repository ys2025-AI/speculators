#!/bin/bash
# Reproduce report: DSpark from-scratch, SWA, sfa=true (default), ce:0.1/tv:0.9, 5ep.
# Usage: bash dspark_repro_scratch_swa.sh
set -uo pipefail
...
LABEL="repro_scratch_swa_sfa_false"
...
DRAFT_CONFIG="/workspace/converted_models/dflash_block7_swa"   # SWA config (sfa=true default)
...
echo "from-scratch | DRAFT_CONFIG=$DRAFT_CONFIG (SWA, sfa=true default) | loss=$LOSS_FN | ep=$EPOCHS"
...
echo "=== Step 3: Training (from-scratch, SWA, sfa=true) ==="
