#!/bin/bash
# DSpark online training: from DFlash checkpoint with SWA config, 4-device 2+2 layout.
# Uses a modified DFlash config (sliding_window=2048) to match from-scratch DSpark's attention type.
...
LOG_DIR="/tmp/dspark_from_dflash_swa_logs"
mkdir -p "$LOG_DIR"
...
DFLASH_CKPT="/tmp/dflash_qwen3_8b_swa"
DATA_PATH="/tmp/dspark_from_dflash_output"
SAVE_PATH="/tmp/dspark_from_dflash_swa_output/checkpoints"
HIDDEN_STATES_PATH="/tmp/dspark_hidden_states_swa"
VLLM_PORT_0=8000
VLLM_PORT_1=8001
...
echo "=== DSpark (from DFlash SWA, 2+2) started at $(date) ==="
echo "DFlash src (SWA): $DFLASH_CKPT"
echo "Save: $SAVE_PATH"

if [ -f "$DATA_PATH/data-00000-of-00001.arrow" ]; then
    echo "=== Step 1: data found, skipping ==="
else
...
echo "=== Step 2a: Launching vLLM #0 (NPU $VLLM_NPUS_0, port $VLLM_PORT_0) ==="
...
cleanup() {
    echo "Stopping vLLM (PIDs $VLLM_PID_0, $VLLM_PID_1)..."
...
wait_for_vllm() {
    ...
    while true; do
        if curl -sf "http://localhost:${port}/v1/models" > /dev/null 2>&1; then
            echo "vLLM ${label} ready (port ${port}, after ${elapsed}s)"
            return 0
        fi
        if ! kill -0 "$pid" 2>/dev/null; then
            echo "ERROR: vLLM ${label} died. Aborting."
            exit 1
        fi
        elapsed=$((elapsed + 2))
        if [ "$elapsed" -ge 600 ]; then
            echo "ERROR: vLLM ${label} not ready in 600s. Aborting."
            exit 1
        fi
        sleep 2
    done
...
echo "=== Step 2b: Launching vLLM #1 (NPU $VLLM_NPUS_1, port $VLLM_PORT_1) ==="
...
echo "=== Step 3: Training (DSpark, from DFlash SWA, 2+2) ==="
...
echo "Done. DSpark (from DFlash SWA) checkpoint saved to $SAVE_PATH"
echo "=== DSpark (from DFlash SWA, 2+2) finished at $(date) ==="
